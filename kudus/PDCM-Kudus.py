...
URL = os.getenv("URL_KUDUS")
TOKEN = os.getenv('TOKEN_KUDUS')
ORG = os.getenv('ORG_KUDUS')
BUCKET = os.getenv('BUCKET_KUDUS')

TIMEZONE=7

localFolder = [
    '/home/shms/ftp/1000-kudus/',
    # '/home/shms/ftp/1000-kudus/2019-10-18.txt',
]
...
class MyEventHandler(pyinotify.ProcessEvent):
    ...
    def process_IN_CLOSE_WRITE(self, event):
        ...
        if event.pathname in fileList == False:
            ...
            # Remove the rows with value > 9999 or < -9999
            df = df[df['value'] < 9999]
            df = df[df['value'] > -9999]

            # Replace the bad values with math.nan
            # df['value'].where(df['value'] < 9999, math.nan, inplace=True)
